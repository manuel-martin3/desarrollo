...
namespace BapFormulariosNet.D70Produccion.UDP
{
    public partial class Frm_diseñador : plantilla
    {
        ...
        private void griddiseñador_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (griddiseñador.CurrentRow != null)
                {
                    ...
                    xdiseñadorid = griddiseñador.Rows[e.RowIndex].Cells["gdisenadorid"].Value.ToString().Trim();
        ...
        private void griddiseñador_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            {
                var xdiseñadorid = string.Empty;
                xdiseñadorid = griddiseñador.Rows[griddiseñador.CurrentRow.Index].Cells["gdisenadorid"].Value.ToString().Trim();
                data_diseñador(xdiseñadorid);
            }
        }
        ...
        {
            form_bloqueado(false);
            var rowdiseñadorid = Tabladiseñador.Select("disenadorid='" + xdiseñadorid + "'");
            if (rowdiseñadorid.Length > 0)
            {
                foreach (DataRow row in rowdiseñadorid)
                {
                    diseñadorid.Text = row["disenadorid"].ToString().Trim();
                    disenadorname.Text = row["disenadorname"].ToString().Trim();
                }
                btn_nuevo.Enabled = true;
