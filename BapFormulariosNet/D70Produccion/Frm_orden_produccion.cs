...
namespace BapFormulariosNet.D70Produccion
{
    public partial class Frm_orden_produccion : plantilla
    {
        ...
        private DataTable TablaOrdenProdMaterial;
        ...
        private String ssModoMa = "";

        private string tipop_ = "";
        private string serop_ = "";
        private string numop_ = "";
        private int partepdaid_ = 0;
        ...
        void TmpOrdenProdColor()
        {
            ...
            TablaOrdenProdColor.Columns.Add("cantot", typeof(Decimal));
        }
        ...
        private void limpiar_documento()
        {
            try
            {
                ...
                cmb_partepda.SelectedIndex = -1;
                productidtela.Text = "";
                productnametela.Text = "";
                txt_ancho.Text = "";
                txt_gramaje.Text = "";
                txt_tipop04.Text = "__";
                txt_serop04.Text = "____";
                txt_numop04.Text = "__________";
                ...
                //if (TablaOrdenProdMaterial.Rows != null)
                //    TablaOrdenProdMaterial.Clear();
        ...
        {
            Double tacotot = 0;
            Double tacon01 = (txt_taco01.Text.ToString().Length) > 0 ? Convert.ToDouble(txt_taco01.Text) : 0;
            Double tacon02 = (txt_taco02.Text.ToString().Length) > 0 ? Convert.ToDouble(txt_taco02.Text) : 0;
            Double tacon03 = (txt_taco03.Text.ToString().Length) > 0 ? Convert.ToDouble(txt_taco03.Text) : 0;
            Double tacon04 = (txt_taco04.Text.ToString().Length) > 0 ? Convert.ToDouble(txt_taco04.Text) : 0;
            Double tacon05 = (txt_taco05.Text.ToString().Length) > 0 ? Convert.ToDouble(txt_taco05.Text) : 0;
            Double tacon06 = (txt_taco06.Text.ToString().Length) > 0 ? Convert.ToDouble(txt_taco06.Text) : 0;
            Double tacon07 = (txt_taco07.Text.ToString().Length) > 0 ? Convert.ToDouble(txt_taco07.Text) : 0;
            Double tacon08 = (txt_taco08.Text.ToString().Length) > 0 ? Convert.ToDouble(txt_taco08.Text) : 0;
            Double tacon09 = (txt_taco09.Text.ToString().Length) > 0 ? Convert.ToDouble(txt_taco09.Text) : 0;
            Double tacon10 = (txt_taco10.Text.ToString().Length) > 0 ? Convert.ToDouble(txt_taco10.Text) : 0;
            Double tacon11 = (txt_taco11.Text.ToString().Length) > 0 ? Convert.ToDouble(txt_taco11.Text) : 0;
            Double tacon12 = (txt_taco12.Text.ToString().Length) > 0 ? Convert.ToDouble(txt_taco12.Text) : 0;
        ...
        private void Data_OrdenProdCab(tb_pp_ordenprod BE)
        {
            ...
            if (rowprodcab.Length > 0)
            {
                foreach (DataRow row in rowprodcab)
                {
                    ...
                    // PINTANDO DATOS EN LA PESTAÑA DE MATERIALES
                    txt_tipop04.Text = "OP";
                    txt_serop04.Text = row["serop"].ToString();
                    txt_numop04.Text = row["numop"].ToString();

                    // CARGAMOS EL DETALLE DE LAS MATERIALES A EDITAR
                    CargarMaterialOrdenProd(txt_tipop04.Text, txt_serop04.Text, txt_numop04.Text);
            ...
            try
            {
                ...
                if (ValidaDatosFase())
                {
                    ...
                    if (ValidaDatosFase2())
                    {
                        ...
                        {
                            CargarFaseOrdenProd("OP", txt_serop03.Text, txt_numop03.Text);
                            //MessageBox.Show("Datos Modificado Correctamente !!!", "Confirmación", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            procesado = true;
                        }
                        else
        ...
        bool ValidaDatosMaterial()
        {
            bool result = true;
            if (txt_tipop04.Text.Length != 2)
            {
                result = false;
                MessageBox.Show("Falta el Tipo de OP", "Confirmación !!!");
            }
            else if (txt_serop04.Text.Length != 4)
            {
                result = false;
                MessageBox.Show("Falta la Serie de OP", "Confirmación !!!");
            }
            else if (txt_numop04.Text.Length != 10)
            {
                result = false;
                MessageBox.Show("Falta la Numero de OP", "Confirmación !!!");
            }
            else if (cmb_partepda.SelectedIndex == -1)
            {
                result = false;
                MessageBox.Show("Seleccione Una Parte de Prenda !!!");
            }
            else if (productidtela.Text.Length == 0)
            {
                result = false;
                MessageBox.Show("Falta La Tela", "Confirmación !!!");
            }
        ...
        void DeleteFase()
        {
            ...
            try
            {
                ...
                if (BL.DeleteFase(EmpresaID, BE))
                {
                    ...
                    LimpiarCeldasMa();
                    BloqueoBotones(false);
                    CargarFaseOrdenProd("OP", txt_serop04.Text, txt_numop04.Text);
                    btn_new.Enabled = true;
                }
        ...
        void CargarMaterialOrdenProd(String xtipdop, String xserdop, String xnumop)
        {
            tb_pp_ordenprod BE = new tb_pp_ordenprod();
            tb_pp_ordenprodBL BL = new tb_pp_ordenprodBL();
            BE.tipop = xtipdop;
            BE.serop = xserdop;
            BE.numop = xnumop;
            TablaOrdenProdMaterial = BL.GetAll_Material(EmpresaID, BE).Tables[0];
            dgv_ordenprodtela.DataSource = TablaOrdenProdMaterial;
        }
        ...
        private void productidtela_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F1)
            {
                AyudaProducto("");
        ...
        private void RecibeProducto(String resultado1, String resultado2, String resultado3, String resultado4, String resultado5)
        {
            try
            {
                ...
                {
                    productidtela.Text = resultado1;
                    productnametela.Text = resultado2;
                    txt_ancho.Text = resultado3;
                    txt_gramaje.Text = resultado4;
                }
            }
            catch (Exception ex)
            {
        ...
        void BloqueoBotonesMa(Boolean var)
        {
            pnl_editMa.Enabled = var;
            pnl_edit2Ma.Enabled = var;
            btn_newMa.Enabled = var;            
            btn_deleteMa.Enabled = var;
            btn_cancelMa.Enabled = var;
            btn_editMa.Enabled = var;
            dgv_ordenprodtela.Enabled = var;
            btn_saveMa.Enabled = var;
        }

        void LimpiarCeldasMa() 
        {
            cmb_partepda.SelectedIndex = -1;
            productidtela.Text = "";
            productnametela.Text = "";
        ...
        private void btn_newMa_Click(object sender, EventArgs e)
        {
            if (XNIVEL == "0")
            {
                if (Equivalencias.Left(txt_tipop04.Text, 1) == "_" ||
                    Equivalencias.Left(txt_serop04.Text, 1) == "_" ||
                    Equivalencias.Left(txt_numop04.Text, 1) == "_")
                {
                    MessageBox.Show("No Existe Numero de Doc...!!!", "Verificar");
                    xtraTabControl1.SelectedTabPage = tab_01;
                }
                ...
                {
                    BloqueoBotonesMa(false);
                    LimpiarCeldasMa();
                    ssModoMa = "NEW";
                    btn_cancelMa.Enabled = true;
                    pnl_editMa.Enabled = true;
                    pnl_edit2Ma.Enabled = true;
                    btn_saveMa.Enabled = true;
                    cmb_partepda.SelectedIndex = -1;
                }
            }
        }

        private void btn_editMa_Click(object sender, EventArgs e)
        {
            //editar de acuerdo al usuario 
            if (XNIVEL == "0") // SEGUN UN USUARIO ADMIN PUEDE TENER ACCESO A UNA MODIFICACION TOTAL
            {
        ...
        private void btn_saveMa_Click(object sender, EventArgs e)
        {
            ...
            if (procesado)
            {
                ...
                BloqueoBotonesMa(false);
                LimpiarCeldasMa();
                btn_newMa.Enabled = true;
                dgv_ordenprodtela.Enabled = true;
            }
        }
        ...
        void InsertMaterial()
        {
            ...
            {
                // PREVIA VALIDACION
                if (ValidaDatosMaterial())
                {
                    BE.tipop = "OP";
                    BE.serop = txt_serop04.Text;
                    BE.numop = txt_numop04.Text;
                    BE.partepdaid = Convert.ToInt32(cmb_partepda.SelectedValue.ToString());
                    BE.productid = productidtela.Text.Trim();
                    BE.ancho = txt_ancho.Text;
                    BE.gramaje = txt_gramaje.Text;
                    BE.estado = "0";
                    BE.usuar = VariablesPublicas.Usuar.ToUpper().Trim();

                    if (BL.InsertMaterial(EmpresaID, BE))
                    {
                        CargarMaterialOrdenProd("OP", txt_serop04.Text, txt_numop04.Text);
                        //MessageBox.Show("Datos Modificado Correctamente !!!", "Confirmación", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        procesado = true;
                    }
                    else
                        MessageBox.Show("Tela no Registrada !!!", "Confirmación", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
        ...
        void UpdateMaterial()
        {
            ...
            try
            {
                ...
                if (ValidaDatosMaterial())
                {
                    ...
                    if (BL.UpdateMaterial(EmpresaID, BE))
                    {
                        CargarMaterialOrdenProd("OP", txt_serop04.Text, txt_numop04.Text);
                        //MessageBox.Show("Datos Modificado Correctamente !!!", "Confirmación", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        procesado = true;
                        ...
                        MessageBox.Show("Tela no Modificada !!!", "Confirmación", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
        ...
        void DeleteMaterial()
        {
            try
            {
                ...
                if (BL.DeleteMaterial(EmpresaID, BE))
                {
                    SEGURIDAD_LOG("E");
                    MessageBox.Show("Datos Eliminado Correctamente !!!", "Confirmación", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    NIVEL_FORMS();
        ...
        private void btn_cancelMa_Click(object sender, EventArgs e)
        {
            BloqueoBotonesMa(false);
            ssModoMa = "EDI";
            btn_newMa.Enabled = true;
        ...
        private void btn_deleteMa_Click(object sender, EventArgs e)
        {
            if (XNIVEL == "0")
            {
                var sw_prosigue = false;
        ...
        private void dgv_ordenprodtela_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            tb_pp_ordenprod BE = new tb_pp_ordenprod();
            tb_pp_ordenprodBL BL = new tb_pp_ordenprodBL();            
            try
            {
                ...
                if (dgv_ordenprodtela.CurrentRow != null)
                {
                    cmb_partepda.SelectedValue = dgv_ordenprodtela.Rows[e.RowIndex].Cells["partepdaid"].Value.ToString().Trim();
                    productidtela.Text = dgv_ordenprodtela.Rows[e.RowIndex].Cells["productid"].Value.ToString().Trim();
                    productnametela.Text = dgv_ordenprodtela.Rows[e.RowIndex].Cells["productname"].Value.ToString().Trim();
                    txt_ancho.Text = dgv_ordenprodtela.Rows[e.RowIndex].Cells["ancho"].Value.ToString().Trim();
                    txt_gramaje.Text = dgv_ordenprodtela.Rows[e.RowIndex].Cells["gramaje"].Value.ToString().Trim();

                    //if (txt_familiatelaname.Text.Trim().Length > 0)
                    //{
                        BE.tipop = txt_tipop04.Text;
                        BE.serop = txt_serop04.Text;
                        BE.numop = txt_numop04.Text;
                        BE.partepdaid = Convert.ToInt32(cmb_partepda.SelectedValue.ToString());
                        BE.productid = productidtela.Text;                                                                  
                        BE.idx = "SER";
                        
                        DataTable dt = new DataTable();
                        dt = BL.LstConsumoTelas(EmpresaID, BE).Tables[0];
                        string arows = dt.Rows[0].Field<string>("Idx");

                            if (arows == "0")
                            {
                                MessageBox.Show("El Producto seleccionado registra cantidad habilitada 0, \n Presione botón Aceptar para continuar ", "Confirmación", 
                                    MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                            } 

                            txt_parte.Text = dgv_ordenprodtela.Rows[e.RowIndex].Cells["partepdaname"].Value.ToString().Trim();
                            txt_productname1.Text = dgv_ordenprodtela.Rows[e.RowIndex].Cells["productname"].Value.ToString().Trim();
                            txt_parte.BackColor = Color.Yellow;
                            txt_productname1.BackColor = Color.Yellow;
                            tipop_ = txt_tipop04.Text;
                            serop_ = txt_serop04.Text;
                            numop_ = txt_numop04.Text;
                            partepdaid_ = Convert.ToInt32(cmb_partepda.SelectedValue.ToString());
                            productid_ = dgv_ordenprodtela.Rows[e.RowIndex].Cells["productid"].Value.ToString().Trim();
                            estado = "NEW";


                            if ((txt_productname1.Text.Trim().Length > 0)&& (txt_parte.Text.Trim().Length > 0))
                            {
                   ...
                   // }
                    //else
                    //{
                    //    MessageBox.Show("Seleccionar un Item !!!", "Confirmación", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    //}
                    ...
                    btn_editMa.Enabled = true;
                    if (XNIVEL == "0")
                        btn_deleteMa.Enabled = true;
                }
        ...
        private void dgv_ordenprodtela_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Up || e.KeyCode == Keys.Down)
            {
                cmb_partepda.SelectedValue = dgv_ordenprodtela.Rows[dgv_ordenprodtela.CurrentRow.Index].Cells["partepdaid"].Value.ToString().Trim();
                productidtela.Text = dgv_ordenprodtela.Rows[dgv_ordenprodtela.CurrentRow.Index].Cells["productid"].Value.ToString().Trim();
                productnametela.Text = dgv_ordenprodtela.Rows[dgv_ordenprodtela.CurrentRow.Index].Cells["productname"].Value.ToString().Trim();
                txt_ancho.Text = dgv_ordenprodtela.Rows[dgv_ordenprodtela.CurrentRow.Index].Cells["ancho"].Value.ToString().Trim();
                txt_gramaje.Text = dgv_ordenprodtela.Rows[dgv_ordenprodtela.CurrentRow.Index].Cells["gramaje"].Value.ToString().Trim();

                txt_productname1.Text = dgv_ordenprodtela.Rows[dgv_ordenprodtela.CurrentRow.Index].Cells["productname"].Value.ToString().Trim();
        ...
        private void Mdi_dgv_ordenproduccion_DoubleClick(object sender, EventArgs e)
        {
            xtraTabControl1.SelectedTabPage = tab_02;
            btn_editar.Enabled = true;
            btn_editar.Enabled = true;
            btn_generar.Enabled = false;
                ...
                txt_parte.Text = "";
                txt_productname1.Text = "";

                txt_ltizado.EditValue=0;
                txt_prendpanos.EditValue=0;
                txt_numpanos.EditValue=0;
                txt_tolerancia.EditValue=0;

                txt_parte.BackColor = Color.White;
                txt_productname1.BackColor = Color.White;

            if (dgv_calconsumo.Rows.Count>0 && dgv_consumoreal.Rows.Count>0)
            {               
                dgv_calconsumo.DataSource=null;
                dgv_calconsumo.Refresh();
                dgv_consumoreal.DataSource = null;
                dgv_consumoreal.Refresh();
            }
           
                         
        }

        private void txt_ltizado_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                txt_prendpanos.Focus();
            }
        }

        private void txt_prendpanos_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                txt_numpanos.Focus();
        ...
        private void txt_numpanos_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
        ...
        private void txt_ltizado_MouseDown(object sender, MouseEventArgs e)
        {
            if (!String.IsNullOrEmpty(ActiveControl.Text))
            {
                txt_ltizado.SelectionStart = 0;
                txt_ltizado.SelectionLength = ActiveControl.Text.Length;
            }
        }

        private void txt_prendpanos_MouseDown(object sender, MouseEventArgs e)
        {
            if (!String.IsNullOrEmpty(ActiveControl.Text))
            {
                txt_prendpanos.SelectionStart = 0;
                txt_prendpanos.SelectionLength = ActiveControl.Text.Length;
            }
        }

        private void txt_numpanos_MouseDown(object sender, MouseEventArgs e)
        {
            if (!String.IsNullOrEmpty(ActiveControl.Text))
            {
                txt_numpanos.SelectionStart = 0;
                txt_numpanos.SelectionLength = ActiveControl.Text.Length;
            }
        }

        private void txt_tolerancia_MouseDown(object sender, MouseEventArgs e)
        {
            if (!String.IsNullOrEmpty(ActiveControl.Text))
            {
                txt_tolerancia.SelectionStart = 0;
                txt_tolerancia.SelectionLength = ActiveControl.Text.Length;
            }
        }

        void gencalculo(string estado)
        {
            tb_pp_ordenprod BE = new tb_pp_ordenprod();
            tb_pp_ordenprodBL BL = new tb_pp_ordenprodBL();
            ...
            try
            {
                if (txt_productname1.Text.Trim().Length > 0 && txt_parte.Text.Trim().Length > 0)
                {
                    BE.tipop = tipop_.ToString();
                    BE.serop = serop_.ToString();
                    BE.numop = numop_.ToString();
                    BE.partepdaid = Convert.ToInt32(partepdaid_.ToString());
                    BE.productid = productid_.ToString();
                    BE.articid = xarticid.ToString();
                    BE.largtizado = Convert.ToDecimal(txt_ltizado.Text);
                    BE.pdasxpano = Convert.ToDecimal(txt_prendpanos.Text);
                    BE.numpanos = Convert.ToInt32(txt_numpanos.Text);
                    BE.porcentolerancia = Convert.ToDecimal(txt_tolerancia.Text);
                    BE.cantreal = Convert.ToInt32(txt_cantreal.Text);
                    BE.usuar = VariablesPublicas.Usuar.ToString();
                    ...
                    if (BL.GenConsumo(EmpresaID, BE))
                    {
                        //Si se Genera Bien el Consumo Cargaremos los Detalles   
                        CargarConsumos("SE1", "SE2");
                        txt_parte.BackColor = Color.YellowGreen;
                        txt_productname1.BackColor = Color.YellowGreen;
                        MessageBox.Show("Transacción Generada Correctamente !!!", "Confirmación", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        MessageBox.Show("Cálculo de consumo no se generó. " +
                            "Parece que ya se ha realizado anteriomente para este mismo producto,  Por favor verifique !!!", "Aviso",
                ...
                {
                    MessageBox.Show("El Producto seleccionado no posee familia asociado  !!!", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
        ...
        {
            if ((txt_parte.Text.Trim().Length>0)&&(txt_productname1.Text.Trim().Length>0))
            {
                gencalculo(estado.ToString());
            }
            else
            {
                MessageBox.Show("¡¡¡Para Realizar el cálculo, Elegir un registro de la lista superior !!!", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            
        }


        void CargarConsumos(string CPO, string DET)
        {
            tb_pp_ordenprod BE = new tb_pp_ordenprod();
            tb_pp_ordenprodBL BL = new tb_pp_ordenprodBL();
            DataTable dt = new DataTable();
            ...
            dt = BL.LstConsumoTelas(EmpresaID, BE).Tables[0];            
            dgv_calconsumo.DataSource = dt;


            DataTable dt2 = new DataTable();
            BE.idx = DET; //"DET"; 
            dt2 = BL.LstConsumoTelas(EmpresaID, BE).Tables[0];
            dgv_consumoreal.DataSource = dt2;
        ...
        private void dgv_consumoreal_CellClick(object sender, DataGridViewCellEventArgs e)
        {

        }

        private void dgv_calconsumo_CellClick(object sender, DataGridViewCellEventArgs e)        
        {
            tb_pp_ordenprod BE = new tb_pp_ordenprod();
            try
            {
                if (dgv_calconsumo.CurrentRow != null)
                {                   
                    ...
                    //{
                        ...
                        partepdaid_ = Convert.ToInt32(dgv_calconsumo.Rows[e.RowIndex].Cells["partepdaid_cpo"].Value.ToString().Trim());
                        txt_parte.Text = dgv_calconsumo.Rows[e.RowIndex].Cells["partepdaname_cpo"].Value.ToString().Trim();

                        txt_productname1.Text = dgv_calconsumo.Rows[e.RowIndex].Cells["productname_cpo"].Value.ToString().Trim();

                        txt_ltizado.Text = dgv_calconsumo.Rows[e.RowIndex].Cells["largtizado"].Value.ToString().Trim();
                        txt_prendpanos.Text = dgv_calconsumo.Rows[e.RowIndex].Cells["pdasxpano"].Value.ToString().Trim();
                        txt_numpanos.Text = dgv_calconsumo.Rows[e.RowIndex].Cells["numpanos"].Value.ToString().Trim();
                        txt_tolerancia.Text = dgv_calconsumo.Rows[e.RowIndex].Cells["porcentolerancia"].Value.ToString().Trim();
                        productid_ = dgv_calconsumo.Rows[e.RowIndex].Cells["productid_cpo"].Value.ToString().Trim();
                        estado="UPD";
                        //partepdaid_ = Convert.ToInt32(cmb_partepda.SelectedValue.ToString());
                        //productid_ = productidtela.Text;
                    //}
                    //else
                    //{
                    //    MessageBox.Show("El Producto seleccionado no posee familia asociado  !!!", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void dgv_calconsumo_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Up || e.KeyCode == Keys.Down)
            {
                //txt_ltizado.Text = dgv_calconsumo.Rows[dgv_calconsumo.CurrentRow.Index].Cells["largtizado"].Value.ToString().Trim();
                //cmb_partepda.SelectedValue = dgv_ordenprodtela.Rows[dgv_ordenprodtela.CurrentRow.Index].Cells["partepdaid"].Value.ToString().Trim();
                //productidtela.Text = dgv_ordenprodtela.Rows[dgv_ordenprodtela.CurrentRow.Index].Cells["productid"].Value.ToString().Trim();
                //productnametela.Text = dgv_ordenprodtela.Rows[dgv_ordenprodtela.CurrentRow.Index].Cells["productname"].Value.ToString().Trim();
                //txt_ancho.Text = dgv_ordenprodtela.Rows[dgv_ordenprodtela.CurrentRow.Index].Cells["ancho"].Value.ToString().Trim();
        ...
        {
            if ((dgv_calconsumo.Rows.Count>0)&((txt_productname1.Text.Trim().Length>0)||(txt_parte.Text.Trim().Length>0)))
            {                           
                if (MessageBox.Show("¿Está seguro que desea eliminar el registro?", "Aviso de Confirmación", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                {
                    gencalculo("DEL");
                }
            }
            ...
            {
                MessageBox.Show("¡¡¡Para eliminar, Elegir un registro de la lista inferior!!!", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
        }

        private void txt_productname1_TextChanged(object sender, EventArgs e)
        {

        }

        private void txt_parte_TextChanged(object sender, EventArgs e)
        {
