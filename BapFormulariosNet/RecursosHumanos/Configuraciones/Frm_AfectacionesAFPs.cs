...
{
    public partial class Frm_AfectacionesAFPs : plantilla
    {
        ...
        public Frm_AfectacionesAFPs()
        {
            InitializeComponent();

            KeyDown += Frm_AfectacionesAFPs_KeyDown;
            FormClosing += Frm_AfectacionesAFPs_FormClosing;
            Load += Frm_AfectacionesAFPs_Load;
            Activated += Frm_AfectacionesAFPs_Activated;
        }

        private void Frm_AfectacionesAFPs_Activated(object sender, EventArgs e)
        {
            if (Sw_LOad)
            {
        ...
        private void Frm_AfectacionesAFPs_Load(object sender, EventArgs e)
        {
            MaximizeBox = false;
        ...
        private void Frm_AfectacionesAFPs_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void CargaDatos()
        {
            ...
            var BL = new tb_plla_afectacionesafpBL();
            var BE = new tb_plla_afectacionesafp();
        ...
        private void Frm_AfectacionesAFPs_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (u_n_opsel > 0)
            {
        ...
        private void Accion(int naccion)
        {
            switch (naccion)
            {
                case 1:
                    ...
                    break;
                case 2:
                    if (!VariablesPublicas.u_Cerrado(VariablesPublicas.EmpresaID, VariablesPublicas.perianio, cboMes.SelectedValue.ToString(), "02", cboTipoplanilla.SelectedValue.ToString(), ""))
                    {
                        u_n_opsel = 2;
                        U_RefrescaControles();
                        if (Examinar.CurrentRow != null)
