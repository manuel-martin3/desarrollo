...
{
    public partial class Frm_AfectacionesRenta5ta : plantilla
    {
        ...
        public Frm_AfectacionesRenta5ta()
        {
            InitializeComponent();

            KeyDown += Frm_AfectacionesRenta5ta_KeyDown;
            FormClosing += Frm_AfectacionesRenta5ta_FormClosing;
            Load += Frm_AfectacionesRenta5ta_Load;
            Activated += Frm_AfectacionesRenta5ta_Activated;
        }

        private void Frm_AfectacionesRenta5ta_Activated(object sender, EventArgs e)
        {
            if (Sw_LOad)
            {
        ...
        private void Frm_AfectacionesRenta5ta_Load(object sender, EventArgs e)
        {
            MaximizeBox = false;
            WindowState = FormWindowState.Normal;
            Examinar.Columns["rubroname"].Width = 300;
            Examinar.Columns["afecto"].HeaderText = ("Afecto" + '\r' + "Quinta");
            Examinar.Columns["gratif"].HeaderText = ("Afecto Mes" + '\r' + "Gratificación");
        ...
        private void CargaDatos()
        {
            ...
            var BL = new tb_plla_afectacionesrta5BL();
            var BE = new tb_plla_afectacionesrta5();
        ...
        private void Frm_AfectacionesRenta5ta_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (u_n_opsel > 0)
            {
        ...
        private void Frm_AfectacionesRenta5ta_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void Accion(int naccion)
        {
            switch (naccion)
            {
                case 1:
                    ...
                    break;
                case 2:
                    if (!VariablesPublicas.u_Cerrado(VariablesPublicas.EmpresaID, VariablesPublicas.perianio, cboMesIni.SelectedValue.ToString(), "03", cmbtipoplanilla.SelectedValue.ToString(), ""))
                    {
                        u_n_opsel = 2;
                        U_RefrescaControles();
                        if (!(Examinar.CurrentRow == null))
                    ...
                    break;
                case 3:
                    if (!VariablesPublicas.u_Cerrado(VariablesPublicas.EmpresaID, VariablesPublicas.perianio, cboMesIni.SelectedValue.ToString(), "03", cmbtipoplanilla.SelectedValue.ToString(), "Eliminar"))
                    {
                        xnomcampo = "";
                        if (!(Examinar.CurrentRow == null))
                        {
                            if (xnomcampo.Length == 0)
                            {
                                ...
                                {
                                    var BL1 = new tb_plla_afectacionesrta5BL();
                                    var BE1 = new tb_plla_afectacionesrta5();

                                    BE1.perianio = spnperiodo.Value.ToString();
                                    BE1.perimes = cboMesIni.SelectedValue.ToString();
                                    ...
                                    if (BL1.Delete(VariablesPublicas.EmpresaID, BE1))
                                    {
                                        U_CancelarEdicion(0);
                                    }
                                    else
        ...
        private void btnNuevaFila_Click(object sender, EventArgs e)
        {
            ...
            for (lc_cont = 0; (lc_cont <= (Tabla.Rows.Count - 1)); lc_cont++)
            {
                if (!(Tabla.Rows[lc_cont]["rubroid"] == DBNull.Value))
                {
