...
{
    public partial class Frm_Planilla_GenMotivosCese : plantilla
    {
        ...
        public Frm_Planilla_GenMotivosCese()
        {
            InitializeComponent();

            KeyDown += Frm_Planilla_GenMotivosCese_KeyDown;
            FormClosing += Frm_Planilla_GenMotivosCese_FormClosing;
            Load += Frm_Planilla_GenMotivosCese_Load;
            Activated += Frm_Planilla_GenMotivosCese_Activated;
        ...
        private void Frm_Planilla_GenMotivosCese_Activated(object sender, EventArgs e)
        {
            if (Sw_LOad)
            {
                U_RefrescaControles();
        ...
        private void Frm_Planilla_GenMotivosCese_Load(object sender, EventArgs e)
        {
        ...
        private void U_RefrescaControles()
        {
            ...
            if ((Examinar.CurrentRow != null))
            {
                xcodcliente = Examinar.Rows[Examinar.CurrentCell.RowIndex].Cells["motfinperiodid"].Value.ToString();
            }
        ...
        private void U_CancelarEdicion(int SwConfirmacion)
        {
            ...
            if (sw_prosigue)
            {
                ...
                if (Examinar.RowCount > 0)
                {
                    for (lc_cont = 0; lc_cont <= Examinar.Rows.Count - 1; lc_cont++)
                    {
                        if (Examinar.Rows[lc_cont].Cells["motfinperiodid"].Value.ToString() == xtmpuser)
                        {
                            Examinar.CurrentCell = Examinar.Rows[lc_cont].Cells["motfinperiodname"];
                            break;
                        }
                    }
                }
        ...
        private void Frm_Planilla_GenMotivosCese_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (u_n_opsel > 0)
            {
        ...
        private void Frm_Planilla_GenMotivosCese_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void btnlog_Click(object sender, EventArgs e)
        {
            if ((Examinar.CurrentRow != null))
            {
                ...
                oform._Nombre = Name;
                oform._ClaveForm = VariablesPublicas.EmpresaID + Examinar.Rows[Examinar.CurrentRow.Index].Cells["motfinperiodid"].Value;
                oform.Owner = this;
                oform.ShowDialog();
            }
        }
        ...
        public void u_PintaDatos()
        {
            if (!Sw_LOad)
            {
                if (TabControl1.SelectedIndex == 1)
                {
                    ...
                    {
                        txtcodigo.Text = Examinar.Rows[Examinar.CurrentRow.Index].Cells["motfinperiodid"].Value.ToString();
                        txtdescripcion.Text = Examinar.Rows[Examinar.CurrentRow.Index].Cells["motfinperiodname"].Value.ToString();
                    }
                }
            }
        }
        ...
        {
            VariablesPublicas.RoleoGrid(Examinar, xtipo, "motfinperiodid");
            u_PintaDatos();
        }
