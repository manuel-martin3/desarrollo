...
{
    public partial class Frm_Planilla_RubrosDescuento : plantilla
    {
        ...
        public Frm_Planilla_RubrosDescuento()
        {
            InitializeComponent();

            KeyDown += Frm_Planilla_RubrosDescuento_KeyDown;
            FormClosing += Frm_Planilla_RubrosDescuento_FormClosing;
            Load += Frm_Planilla_RubrosDescuento_Load;
            Activated += Frm_Planilla_RubrosDescuento_Activated;
        ...
        private void Frm_Planilla_RubrosDescuento_Load(object sender, EventArgs e)
        {
        ...
        private void U_RefrescaControles()
        {
            ...
            chkvisibleq.Enabled = u_n_opsel > 0;
            chkeditableq.Enabled = u_n_opsel > 0;
        ...
        private void btnnuevo_Click(object sender, EventArgs e)
        {
            ...
            var BL = new tb_plla_rubrosdescuentosBL();
            var BE = new tb_plla_rubrosdescuentos();
            BE.tipo = GlobalVars.GetInstance().TipoRubroPlanillaDescuento;
            ...
            txtsigla.Text = GlobalVars.GetInstance().TipoRubroPlanillaDescuento;
        ...
        private void btngrabar_Click(object sender, EventArgs e)
        {
            if (U_Validacion())
            {
                ...
                if (u_n_opsel == 1)
                {
                    ...
                    if (tmpcursor.Rows.Count > 0)
                    {
                        var BLm = new tb_plla_rubrosdescuentosBL();
                        var BEm = new tb_plla_rubrosdescuentos();
                        BEm.tipo = GlobalVars.GetInstance().TipoRubroPlanillaDescuento;
                ...
                tmpcursor.Rows[0]["visibleq"] = (chkvisibleq.Checked ? 1 : 0);
                tmpcursor.Rows[0]["editaq"] = (chkeditableq.Checked ? 1 : 0);
                tmpcursor.Rows[0]["rubropdt"] = cmbrubrorpts.SelectedValue;
                tmpcursor.Rows[0]["status"] = (chkactivo.Checked ? '0' : '9');
                tmpcursor.AcceptChanges();
                var BLIU = new tb_plla_rubrosdescuentosBL();
        ...
        private void Frm_Planilla_RubrosDescuento_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (u_n_opsel > 0)
            {
        ...
        private void Frm_Planilla_RubrosDescuento_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (!Sw_LOad)
            {
                if (TabControl1.SelectedIndex == 1)
                {
                    if ((Examinar.CurrentRow != null) & !(u_n_opsel == 1))
                    {
                        ...
                        chkeditableq.Checked = Convert.ToBoolean(Examinar.Rows[Examinar.CurrentRow.Index].Cells["editaq"].Value);
                        chkvisibleq.Checked = Convert.ToBoolean(Examinar.Rows[Examinar.CurrentRow.Index].Cells["visibleq"].Value);
