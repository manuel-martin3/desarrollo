...
{
    public partial class Frm_Planilla_GenMotivosPermiso : plantilla
    {
        ...
        public Frm_Planilla_GenMotivosPermiso()
        {
            InitializeComponent();

            KeyDown += Frm_Planilla_GenMotivosPermiso_KeyDown;
            FormClosing += Frm_Planilla_GenMotivosPermiso_FormClosing;
            Load += Frm_Planilla_GenMotivosPermiso_Load;
            Activated += Frm_Planilla_GenMotivosPermiso_Activated;
        ...
        private void Frm_Planilla_GenMotivosPermiso_Activated(object sender, EventArgs e)
        {
            if (Sw_LOad)
            {
                U_RefrescaControles();
        ...
        private void Frm_Planilla_GenMotivosPermiso_Load(object sender, EventArgs e)
        {
        ...
        private void CargaDatos()
        {
            ...
            {
                xcodcliente = Examinar.Rows[Examinar.CurrentRow.Index].Cells["cmotivo"].Value.ToString();
            }
            ...
            BE.norden = 1;
            BE.ver_blanco = 0;
            tablaclientes = BL.GetAllMotivosPermiso(VariablesPublicas.EmpresaID, BE).Tables[0];
            ...
            {
                Examinar.Sort(Examinar.Columns["cmotivo"], System.ComponentModel.ListSortDirection.Ascending);
            }
            if (Examinar.CurrentRow == null)
            {
                if (Examinar.RowCount > 0)
                {
                    Examinar.CurrentCell = Examinar.Rows[0].Cells["cmotivo"];
                }
            }
            for (lc_cont = 0; lc_cont <= Examinar.Rows.Count - 1; lc_cont++)
            {
                if (Examinar.Rows[lc_cont].Cells["cmotivo"].Value.ToString() == xcodcliente)
                {
                    Examinar.CurrentCell = Examinar.Rows[lc_cont].Cells["cmotivo"];
                    break;
                }
            }
        }
        ...
        private void U_RefrescaControles()
        {
            ...
            if ((Examinar.CurrentRow != null))
            {
                xcodcliente = Examinar.Rows[Examinar.CurrentCell.RowIndex].Cells["cmotivo"].Value.ToString();
            }
        ...
        private void btnnuevo_Click(object sender, EventArgs e)
        {
            ...
            var BL = new tb_plla_tab0100BL();
            var BE = new tb_plla_tab0100();
            txtcodigo.Text = BL.MotivosPermiso_MAXCODIGO(VariablesPublicas.EmpresaID.ToString(), BE).Tables[0].Rows[0]["maximo_codigo"].ToString();
        ...
        private void U_CancelarEdicion(int SwConfirmacion)
        {
            ...
            if (sw_prosigue)
            {
                ...
                if (Examinar.RowCount > 0)
                {
                    for (lc_cont = 0; lc_cont <= Examinar.Rows.Count - 1; lc_cont++)
                    {
                        if (Examinar.Rows[lc_cont].Cells["cmotivo"].Value.ToString() == xtmpuser)
                        {
                            Examinar.CurrentCell = Examinar.Rows[lc_cont].Cells["cmotivo"];
                            break;
                        }
        ...
        private void btngrabar_Click(object sender, EventArgs e)
        {
            if (U_Validacion())
            {
                ...
                BE.norden = 1;
                BE.ver_blanco = 0;
                tmpcursor = BL.GetAllMotivosPermiso(VariablesPublicas.EmpresaID, BE).Tables[0];
                if (u_n_opsel == 1)
                {
                    while (1 == 1)
                    {
                        ...
                        if (tmpcursor.Rows.Count > 0)
                        {
                            ...
                            txtcodigo.Text = BLMX.MotivosPermiso_MAXCODIGO(VariablesPublicas.EmpresaID.ToString(), BEMX).Tables[0].Rows[0]["maximo_codigo"].ToString();
                        }
                        else
                        {
                            break;
                    ...
                    tmpcursor.Rows[0]["cmotivo"] = txtcodigo.Text.Trim();
                }
                tmpcursor.Rows[0]["dmotivo"] = txtdescripcion.Text.Trim();
                tmpcursor.AcceptChanges();
                if (BL.MotivosPermiso_InsertUpdate(VariablesPublicas.EmpresaID, BE, tmpcursor))
                {
                    seguridadlog();
                    U_CancelarEdicion(0);
                }
        ...
        private void btneliminar_Click(object sender, EventArgs e)
        {
            if ((Examinar.CurrentRow != null))
            {
                ...
                var BL = new tb_plla_tab0100BL();
                var BE = new tb_plla_tab0100();
                BE.codigo = Examinar.Rows[Examinar.CurrentRow.Index].Cells["cmotivo"].Value.ToString();
                tmpcursor = BL.MotivosPermiso_IR(VariablesPublicas.EmpresaID, BE).Tables[0];
                if (BL.Sql_Error.Length > 0)
                {
                ...
                if (xnomcampo.Length == 0)
                {
                    ...
                    BECO.codigo = Examinar.Rows[Examinar.CurrentRow.Index].Cells["cmotivo"].Value.ToString();
                    BECO.norden = 1;
                    BECO.ver_blanco = 0;
                    tmptabla = BLCO.GetAllMotivosPermiso(VariablesPublicas.EmpresaID, BECO).Tables[0];
                    if (BLCO.Sql_Error.Length == 0)
                    {
                        var message = "Desea eliminar datos  " + tmptabla.Rows[0]["cmotivo"].ToString().Trim() + "-" + tmptabla.Rows[0]["dmotivo"].ToString().Trim() + " ...?";
                        ...
                        {
                            BL.MotivosPermiso_ELIMINAR(VariablesPublicas.EmpresaID, BE);
                            if (BL.Sql_Error.Length == 0)
                            {
                                ...
                                BEL.moduloid = Name;
                                BEL.clave = VariablesPublicas.EmpresaID + Examinar.Rows[Examinar.CurrentRow.Index].Cells["cmotivo"].Value.ToString();
                                BEL.cuser = VariablesPublicas.Usuar;
                                BEL.fecha = DateTime.Now;
                                BEL.pc = VariablesPublicas.userip;
                                BEL.accion = "B";
                                BEL.detalle = "Motivo: " + txtdescripcion.Text;
        ...
        private void Frm_Planilla_GenMotivosPermiso_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (u_n_opsel > 0)
            {
        ...
        private void Frm_Planilla_GenMotivosPermiso_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        public void u_PintaDatos()
        {
            if (!Sw_LOad)
            {
                if (TabControl1.SelectedIndex == 1)
                {
                    ...
                    {
                        txtdescripcion.Text = Examinar.Rows[Examinar.CurrentRow.Index].Cells["dmotivo"].Value.ToString();
                        txtcodigo.Text = Examinar.Rows[Examinar.CurrentRow.Index].Cells["cmotivo"].Value.ToString();
                    }
                }
            }
        }
        ...
        {
            VariablesPublicas.RoleoGrid(Examinar, xtipo, "cmotivo");
            u_PintaDatos();
        }
