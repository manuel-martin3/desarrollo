...
{
    public partial class Frm_AyudaCCostoPlla : Form
    {
        public delegate void PasaCentroCostoDelegate(string codigo, string nombre);

        public PasaCentroCostoDelegate _PasaDelegado;
        private bool Inicializa = true;

        public Frm_AyudaCCostoPlla()
        {
            InitializeComponent();

            Load += Frm_AyudaCCostoPlla_Load;
            KeyDown += Frm_AyudaCCostoPlla_KeyDown;
            Activated += Frm_AyudaCCostoPlla_Activated;

            txtdescripcion.GotFocus += new System.EventHandler(txtdescripcion_GotFocus);
        }

        private void Frm_AyudaCCostoPlla_Activated(object sender, EventArgs e)
        {
            if (Inicializa)
            {
                Inicializa = false;
        ...
        private void Frm_AyudaCCostoPlla_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void Frm_AyudaCCostoPlla_Load(object sender, EventArgs e)
        {
        ...
        public void u_SeleccionaRegistro()
        {
            if ((dgProveedor.CurrentRow != null))
            {
                ...
                {
                    _PasaDelegado(dgProveedor.Rows[dgProveedor.CurrentCell.RowIndex].Cells["cencosid"].Value.ToString(), dgProveedor.Rows[dgProveedor.CurrentCell.RowIndex].Cells["cencosname"].Value.ToString());
                }
                Close();
            }
        }
