...
{
    public partial class Form_help_gridproducto_vlx : Form
    {
        ...
        public Form_help_gridproducto_vlx()
        {
            InitializeComponent();
            checkboxColumn();
        ...
        private void _recarga()
        {
            ...
            gridgeneral.Columns["precventa"].ReadOnly = true;
            gridgeneral.Columns["local"].ReadOnly = true;
            gridgeneral.Columns["nostock"].ReadOnly = true;
        }
            ...
            try
            {
                foreach (DataGridViewRow fila in gridgeneral.Rows)
                {
                    ...
                    if (xselec == true)
                    {
                        ...
                        row["precventa"] = gridgeneral.Rows[fila.Index].Cells["precventa"].Value;
                        row["unmed"] = gridgeneral.Rows[fila.Index].Cells["unmed"].Value;
                        if (VariablesPublicas.Moduloid == "0500")
                        {
                            row["nserie"] = gridgeneral.Rows[fila.Index].Cells["nserie"].Value;
                        }

                        if (gridgeneral.Rows[fila.Index].Cells["nostock"].Value.ToString() != string.Empty)
                        {
                            _nostock = (Boolean)gridgeneral.Rows[fila.Index].Cells["nostock"].Value;
                        }
                        else
                        {
                            _nostock = false ;
        ...
        private void Frm_help_general_Load(object sender, EventArgs e)
        {
            ...
            Tabladet.Columns.Add("unmed", typeof(String));
            Tabladet.Columns.Add("stock", typeof(Decimal));
            Tabladet.Columns["stock"].DefaultValue = 0;
            Tabladet.Columns.Add("precventa", typeof(Decimal));
            Tabladet.Columns["precventa"].DefaultValue = 0;
            Tabladet.Columns.Add("local", typeof(String));
            Tabladet.Columns.Add("nserie", typeof(String));
            Tabladet.Columns.Add("nostock", typeof(String));
