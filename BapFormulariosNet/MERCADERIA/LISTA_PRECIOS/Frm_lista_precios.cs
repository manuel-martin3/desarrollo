...
namespace BapFormulariosNet.MERCADERIA.LISTA_PRECIOS
{
    public partial class Frm_lista_precios : plantilla
    {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    frmayuda.sqlquery = "SELECT " +
                                        ...
                                        " LEFT JOIN tb_me_linea L on S.lineaid = L.lineaid AND s.moduloid = l.moduloid ";
                    frmayuda.sqlinner = " LEFT JOIN tb_me_grupo G on S.grupoid = G.grupoid AND s.moduloid = g.moduloid ";
                    frmayuda.sqlwhere = "where S.moduloid=" + modulo;
                    frmayuda.sqland = "and ";
        ...
        private void RecibeSubgrupo(String[] resultados)
        {
            ...
            {
                articid.Text = resultados[0];
                articname.Text = resultados[5];
            }
        }
        ...
        private void ExportCatalog_Click(object sender, EventArgs e)
        {
            ...
            BE.filtro = "5";
            dt = BL.GetAll(EmpresaID, BE).Tables[0];
            ExportarProdCatalog(dt);
        }

        private void ExportarProdCatalog(DataTable TablaLista)
        {
            Excel.Application oXL;
            Excel._Workbook oWB;
            Excel._Worksheet oSheet;
            ...
            try
            {
                ...
                var IndiceFila = 2;
                oSheet.Range["A:B"].NumberFormat = "@";
                oSheet.Range["C:D"].NumberFormat = "_ * #,##0.00_ ;_ * -#,##0.00_ ;_ * -??_ ;_ @_ ";

                foreach (DataRow row in TablaLista.Rows)
                {
                    oSheet.Cells[IndiceFila, 01].Value = "'" + row["codigo"].ToString().Trim();
                    oSheet.Cells[IndiceFila, 02].Value = "'" + row["denominacion"].ToString().Trim();
                    oSheet.Cells[IndiceFila, 03].Value = row["precunit1"];
                    oSheet.Cells[IndiceFila, 04].Value = row["precunit2"];
