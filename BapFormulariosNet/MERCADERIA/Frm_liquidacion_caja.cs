...
namespace BapFormulariosNet.MERCADERIA
{
    public partial class Frm_liquidacion_caja : plantilla
    {
        ...
        private void Frm_liquidacion_caja_Load(object sender, EventArgs e)
        {
            ...
            fecha.Properties.Mask.MaskType = DevExpress.XtraEditors.Mask.MaskType.DateTime;
            fecha.Properties.Mask.EditMask = "dd/MM/yyyy";
        ...
        private void _CalcularTotales(String Column_Name)
        {
            ...
            {
                saldomn.Text = (xsum10mn + xsum20mn).ToString("###,###,##0.#0");
            }
            else
            {
                saldome.Text = (xsum10mn + xsum20mn).ToString("###,###,##0.#0");
            }
        }
        ...
        private void _CargarVentas(DateTime fecha)
        {
            ...
            if (Dt_glosa.Rows.Count > 0)
            {
                for (var j = 0; j < Dt_glosa.Rows.Count; j++)
                {
                    ...
                    var sigla = Dt_glosa.Rows[j][0].ToString();
                    for (var i = 0; i < Dt_name.Rows.Count; i++)
                    {
                        if (sigla.Trim() == Dt_name.Rows[i][1].ToString())
                        {
                            ...
                            decimal de = Convert.ToDecimal(Dt_glosa.Rows[j][2].ToString());
