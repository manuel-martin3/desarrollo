...
namespace BapFormulariosNet.MERCADERIA
{
    public partial class Frm_ventas : plantilla
    {
        ...
        private void form_cargar_datos(String posicion)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    cboTipoperacionid.SelectedValue = tip_operacion;
                ...
                if (dt.Rows.Count > 0)
                {
                    if (dt.Rows[0]["moduloshort"].ToString().Trim().Length == 2)
                    {
                        ...
                        frmayuda.sqlquery = frmayuda.sqlquery + " LS.stock, " + " LI.nostock  " +
                                                                " FROM   tb_me_local_stock  AS LS ";

                        frmayuda.sqlinner = " INNER JOIN tb_me_productos PR " +
                                               " ON  PR.moduloid = LS.moduloid " +
                                               " AND PR.productid = LS.productid " +
                               " INNER JOIN tb_me_listapreciosdet LP " +
                                   " ON  LP.listaprecid = '" + lstListaprec.SelectedValue.ToString() + "' " +
                                   " AND LP.codigo = LEFT(LS.productid,10) " +
                               " LEFT JOIN tb_me_linea li ON PR.lineaid = li.lineaid and PR.moduloid = li.moduloid";
                ...
                if (dt.Rows.Count > 0)
                {
                    if (dt.Rows[0]["moduloshort"].ToString().Trim().Length == 2)
                    {
                        ...
                        frmayuda.sqlinner = " Left Join tb_me_local_stock tb2 on tb1.moduloid=tb2.moduloid and tb1.productid = tb2.productid " +
                                            " LEFT join tb_me_linea li on tb1.lineaid = li.lineaid and tb1.moduloid = li.moduloid ";
        ...
        private bool cabecera_valida()
        {
            ...
            if (cboTipoperacionid.SelectedIndex==-1)
            {
                MessageBox.Show("Seleccione Tipo de Operación !!!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                lSigo = false;
            }
            return lSigo;
        }
        ...
        private void Insert()
        {
            ...
            try
            {
                if (cabecera_valida())
                {
                    ...
                    var Detalle = new tb_me_movimientos.Item();
                    var ListaItems = new List<tb_me_movimientos.Item>();

                    var Detalle2 = new tb_me_movimientos.Tarjetas();
                    var ListaTarjetas = new List<tb_me_movimientos.Tarjetas>();
                    ...
                    BE.tipoperacionid = cboTipoperacionid.SelectedValue.ToString();
                    ...
                    if (TablaTarjetas.Rows.Count > 0)
                    {
                        ...
                        BE.Tarjimporte1 = Convert.ToDecimal(importtarj.Text);
                    }
                    ...
                    foreach (DataRow fila in Tabladetallemov.Rows)
                    {
                        ...
                        Detalle.incprec = Equivalencias.Left(cbo_incprec.Text.Trim(), 1);
                        Detalle.igv = igv;
                    ...
                    {
                        Detalle2 = new tb_me_movimientos.Tarjetas();
                        ...
                        Detalle2.tarjimporte1 = Convert.ToDecimal(fila["tarjetaimpo"].ToString());
                        Detalle2.tarjimporte2 = Convert.ToDecimal(fila["tarjetaimpo"].ToString()) / Convert.ToDecimal(tcamb.Text);
                        Detalle2.status = "0";
                        Detalle2.usuar = VariablesPublicas.Usuar.ToString();


                        if (Convert.ToDecimal(fila["tarjetaimpo"].ToString()) > 0)
                        {
        ...
        private void Update()
        {
            ...
            try
            {
                if (cabecera_valida())
                {
                    ...
                    BE.Tarjimporte1 = Convert.ToDecimal(importtarj.Text);
                    BE.Tarjimporte2 = Convert.ToDecimal(importtarj.Text) / Convert.ToDecimal(tcamb.Text);
        ...
        private void Fill_cboTipoperacionid()
        {
            ...
            BE.almacenaccionid = almacaccionid;

            // Creo una lista de elementos para ponerlos en el combo y le pongo la primera vacia
            ...
            try
            {
                //convierto un miembro del dataset a un datareader
                DataSet ds = BL.GetAll(EmpresaID, BE);
                IDataReader drd = ds.CreateDataReader(ds.Tables[0]);
                ...
                while (drd.Read())
                {
                    ...
                    switch (tipodoc.SelectedValue.ToString())
                    {
                        ...
                        case "FA":
                        case "ND":
                            if ("01".IndexOf(drd["codigoid"].ToString()) != -1)
                            {
                                lAgregar = true;
                            }
                            break;
                        case "NC":
                            if ("05_99".IndexOf(drd["codigoid"].ToString()) != -1)
                            {
                                lAgregar = true;
                            }
                            break;
                    ...
                    if (lAgregar)
                    {
                        ...
                        UnElemento.Name = drd["descripcion"].ToString();
                        UnElemento.Value = drd["codigoid"].ToString();
                        Lista.Add(UnElemento);
                    }
                }

                cboTipoperacionid.DataSource = Lista;
                cboTipoperacionid.DisplayMember = "Name";
                cboTipoperacionid.ValueMember = "Value";
                cboTipoperacionid.SelectedIndex = 0;
        ...
        private void data_TablaTarjetas()
        {
            try
            {
                var BL = new tb_me_movimientostarjBL();
                var BE = new tb_me_movimientostarj();
                var dt = new DataTable();
                BE.moduloid = modulo;
                BE.local = local;
                BE.tipodoc = tipodoc.SelectedValue.ToString().Trim();
                ...
                foreach (DataRow fila in dt.Rows)
                {
                    ...
                    row["tarjetaimpo"] = fila["tarjimporte1"].ToString();
        ...
        private void data_Tabladetallemovmov()
        {
            try
            {
                ...
                foreach (DataRow fila in dt.Rows)
                {
                    var BL2 = new tb_me_local_stockBL();
                    var BE2 = new tb_me_local_stock();
                    var dt2 = new DataTable();
                    BE2.moduloid = modulo;
