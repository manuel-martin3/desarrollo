...
namespace BapFormulariosNet.MERCADERIA
{
    public partial class Frm_AperturaCaja : plantilla
    {
        ...
        private bool QueryExec(String Query)
        {
            var conex = new ConexionDA();
            using (var cnx = new SqlConnection(conex.empConexion(EmpresaID)))
            {
        ...
        private void _CargarVentas(DateTime fecha)
        {
            ...
            var Query = " " +
                        "SELECT tipodoc AS sigla " +
                        "	,('DE : ' + MIN(numdoc) + ' - ' + MAX(numdoc)) glosa " +
                        "	,sum(IIF(tmm.moneda = 'S', totimporte, 0)) importe1 " +
                        "	,sum(IIF(tmm.moneda = '$', totimporte, 0)) importe2 " +
                        "FROM tb_me_movimientoscab AS tmm " +
                        "WHERE tmm.status<>'9' and tmm.moduloid = '" + modulo + "' " +
                        "	AND tmm.LOCAL = '" + local + "' " +
                        "	AND tmm.tipodoc NOT IN ( " +
                        "		'GI' " +
                        "		,'GS' " +
                        "		,'II' " +
                        "		) " +
                        ...
                        "SELECT tt.sigla " +
                        "	,'-' AS glosa " +
                        "	,SUM(tmt.tarjimporte1) AS importe1 " +
                        "	,SUM(tmt.tarjimporte2) AS importe2 " +
                        "FROM tb_me_movimientostarj AS tmt " +
                        "JOIN tb_tarjeta AS tt ON tt.tarjetaid = tmt.tarjetaid " +
                        "JOIN tb_me_movimientoscab AS tmm ON tmm.moduloid = tmt.moduloid " +
                        "	AND tmm.[local] = tmt.[local] " +
                        "	AND tmm.tipodoc = tmt.tipodoc " +
                        "	AND tmm.serdoc = tmt.serdoc " +
                        "	AND tmm.numdoc = tmt.numdoc " +
                        "WHERE tmm.status<>'9' and tmm.moduloid = '" + modulo + "' " +
                        "	AND tmm.[local] = '" + local + "' " +
                        "	AND tmm.fechdoc = '" + fecha.ToString("yyyyMMdd") + "' " +
                        "GROUP BY tt.sigla " +
            ...
            var Query2 = " " +
                        ...
                        "	,fila " +
                        "FROM tb_t1_cajaconcepto AS ttc " +
                        "WHERE charindex(LEFT(conceptoid, 2), '02_03')>0 " +
                        "	AND LEN(conceptoid) > 2 ";
            ...
            if (Dt_glosa.Rows.Count > 0)
            {
                for (var j = 0; j < Dt_glosa.Rows.Count; j++)
                {
                    ...
                    sigla = Dt_glosa.Rows[j][0].ToString();

                    for (var i = 0; i < Dt_name.Rows.Count; i++)
                    {
                        if (sigla.Trim() == Dt_name.Rows[i][1].ToString())
                        {
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("¿Desea Eliminar Todos los Registros ?", "Blue Ocean", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == DialogResult.Yes)
            {
                Query = " Delete From tb_t1_cajadet where moduloid = '" + modulo + "' and local = '" + local + "' ";
                QueryExec(Query);

                Query = " Delete From tb_t1_cajacab where moduloid = '" + modulo + "' and local = '" + local + "' ";
                QueryExec(Query);
