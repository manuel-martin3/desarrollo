...
{
    public class tb_co_OperacionescontabilidadDA
    {
        ...
        public bool Insert(string empresaid, tb_co_Operacionescontabilidad BE)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoOperacionescontabilidad_INSERT", cnx))
                {
        ...
        public bool Update(string empresaid, tb_co_Operacionescontabilidad BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoOperacionescontabilidad_UPDATE", cnx))
                {
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@codigoid", SqlDbType.Char, 3).Value = BE.codigoid;
        ...
        public bool Delete(string empresaid, tb_co_Operacionescontabilidad BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoOperacionescontabilidad_DELETE", cnx))
                {
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@codigoid", SqlDbType.Char, 3).Value = BE.codigoid;
        ...
        public DataSet GetAll(string empresaid, tb_co_Operacionescontabilidad BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoOperacionescontabilidad_SEARCH", cnx))
                {
        ...
        public DataSet GetOne(string empresaid, string codigoid)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoOperacionescontabilidad_SELECT", cnx))
                {
                    DataSet ds = new DataSet();
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
