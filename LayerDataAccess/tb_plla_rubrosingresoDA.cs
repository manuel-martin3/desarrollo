...
{
    public class tb_plla_rubrosingresoDA
    {
        ...
        public bool Insert_Update(string empresaid, DataTable TablaDatos)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("gspTbPllaRubrosingreso_InsertUpdate", cnx))
                    {
        ...
        public DataSet GetAll_CONSULTA(string empresaid, tb_plla_rubrosingreso BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbPllaRubrosingreso_CONSULTA", cnx))
                {
                    DataSet ds = new DataSet();
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
