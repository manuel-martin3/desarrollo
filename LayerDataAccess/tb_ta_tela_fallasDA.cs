...
{
    public class tb_ta_tela_fallasDA
    {
        ...
        public bool Insert(string empresaid, tb_ta_tela_fallas BE)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("gspTbTaTelaFallas_INSERT", cnx))
                {
        ...
        public bool Update(string empresaid, tb_ta_tela_fallas BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbTaTelaFallas_UPDATE", cnx))
                {
        ...
        public bool Delete(string empresaid, tb_ta_tela_fallas BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbTaTelaFallas_DELETE", cnx))
                {
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@fallaid", SqlDbType.Char, 2).Value = BE.fallaid;
        ...
        public DataSet GetAll(string empresaid, tb_ta_tela_fallas BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbTaTelaFallas_SEARCH", cnx))
                {
        ...
        public DataSet GetOne(string empresaid, string fallaid)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbTaTelaFallas_SELECT", cnx))
                {
                    DataSet ds = new DataSet();
