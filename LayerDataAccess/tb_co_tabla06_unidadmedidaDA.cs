...
{
    public class tb_co_tabla06_unidadmedidaDA
    {
        ...
        public bool Insert(string empresaid, tb_co_tabla06_unidadmedida BE)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoTabla06Unidadmedida_INSERT", cnx))
                {
        ...
        public bool Update(string empresaid, tb_co_tabla06_unidadmedida BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoTabla06Unidadmedida_UPDATE", cnx))
                {
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
        ...
        public bool Delete(string empresaid, tb_co_tabla06_unidadmedida BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoTabla06Unidadmedida_DELETE", cnx))
                {
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
        ...
        public DataSet GetAll(string empresaid, tb_co_tabla06_unidadmedida BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoTabla06Unidadmedida_SEARCH", cnx))
                {
        ...
        public DataSet GetOne(string empresaid, string codigoid)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoTabla06Unidadmedida_SELECT", cnx))
                {
                    DataSet ds = new DataSet();
                    {
                        cmd.CommandTimeout = 0;
