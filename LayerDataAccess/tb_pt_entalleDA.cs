...
namespace LayerDataAccess
{
    public class tb_pt_entalleDA
    {
        ...
        public bool Update(string empresaid, tb_pt_entalle BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbPtEntalle_UPDATE", cnx))
                {
        ...
        public DataSet GetAll(string empresaid, tb_pt_entalle BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbPtEntalle_SEARCH", cnx))
                {
        ...
        public DataSet GetOne(string empresaid, string entalleid)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbPtEntalle_SELECT", cnx))
                {
                    DataSet ds = new DataSet();

                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@entalleid", SqlDbType.Char, 4).Value = entalleid;
                    }
