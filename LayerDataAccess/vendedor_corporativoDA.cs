...
{
    public class vendedor_corporativoDA
    {
        ...
        public bool Insert(string empresaid, tb_vendedor_corporativo BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbVendedorCorporativo_INSERT", cnx))
                {
                    ...
                    cmd.Parameters.Add("@vendordni", SqlDbType.Char, 11).Value = BE.vendordni;
                    cmd.Parameters.Add("@vendordire", SqlDbType.VarChar, 100).Value = BE.vendordire;
