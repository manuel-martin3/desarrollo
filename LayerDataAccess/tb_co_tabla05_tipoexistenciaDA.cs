...
{
    public class tb_co_tabla05_tipoexistenciaDA
    {
        ...
        public bool Insert(string empresaid, tb_co_tabla05_tipoexistencia BE)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoTabla05Tipoexistencia_INSERT", cnx))
                {
        ...
        public bool Update(string empresaid, tb_co_tabla05_tipoexistencia BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoTabla05Tipoexistencia_UPDATE", cnx))
                {
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
        ...
        public bool Delete(string empresaid, tb_co_tabla05_tipoexistencia BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoTabla05Tipoexistencia_DELETE", cnx))
                {
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
        ...
        public DataSet GetAll(string empresaid, tb_co_tabla05_tipoexistencia BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoTabla05Tipoexistencia_SEARCH", cnx))
                {
        ...
        public DataSet GetOne(string empresaid, string codigoid)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoTabla05Tipoexistencia_SELECT", cnx))
                {
                    DataSet ds = new DataSet();
                    {
                        cmd.CommandTimeout = 0;
