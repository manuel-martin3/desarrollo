...
{
    public class tb_co_rubroventasDA
    {
        ...
        public bool Insert(string empresaid, tb_co_rubroventas BE)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoRubroventas_INSERT", cnx))
                {
        ...
        public bool Update(string empresaid, tb_co_rubroventas BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoRubroventas_UPDATE", cnx))
                {
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
        ...
        public bool Delete(string empresaid, tb_co_rubroventas BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoRubroventas_DELETE", cnx))
                {
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
        ...
        public DataSet GetAll(string empresaid, tb_co_rubroventas BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoRubroventas_SEARCH", cnx))
                {
        ...
        public DataSet GetAll_IR(string empresaid, tb_co_rubroventas BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoRubroventas_SEARCH_IR", cnx))
                {
                    DataSet ds = new DataSet();
                    {
                        cmd.CommandTimeout = 0;
        ...
        public DataSet GetOne(string empresaid, string rubroid)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoRubroventas_SELECT", cnx))
                {
                    DataSet ds = new DataSet();
                    {
                        cmd.CommandTimeout = 0;
