...
{
    public class tb_pt_subcoleccionDA
    {
        ...
        public bool Insert(string empresaid, tb_pt_subcoleccion BE)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("gspTbPtSubColeccion_INSERT", cnx))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("@subcoleccionid", SqlDbType.Char, 2).Value = BE.subcoleccionid;
                    cmd.Parameters.Add("@subcoleccionname", SqlDbType.VarChar, 30).Value = BE.subcoleccionname;
        ...
        public bool Update(string empresaid, tb_pt_subcoleccion BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbPtSubColeccion_UPDATE", cnx))
                {
        ...
        public DataSet GetAll(string empresaid, tb_pt_subcoleccion BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbPtSubColeccion_SEARCH", cnx))
                {
        ...
        public DataSet GetOne(string empresaid, string subcoleccionid)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbPtSubColeccion_SELECT", cnx))
                {
                    DataSet ds = new DataSet();

                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@subcoleccionid", SqlDbType.Char, 3).Value = subcoleccionid;
                    }
