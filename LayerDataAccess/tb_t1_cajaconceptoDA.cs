...
{
    public class tb_t1_cajaconceptoDA
    {
        ...
        public bool Insert(string empresaid, tb_t1_cajaconcepto BE)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("gspTbt1_cajaconcepto_INSERT", cnx))
                {
        ...
        public bool Update(string empresaid, tb_t1_cajaconcepto BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbt1_cajaconcepto_UPDATE", cnx))
                {
        ...
        public bool Delete(string empresaid, tb_t1_cajaconcepto BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbt1_cajaconcepto_DELETE", cnx))
                {
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        //cmd.Parameters.Add("@idcla", SqlDbType.VarChar, 40).Value = BE.idcla;
        ...
        public DataSet GetAll(string empresaid, tb_t1_cajaconcepto BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbT1Cajaconcepto_SEARCH", cnx))
                {
        ...
        public DataSet GetOne(string empresaid, tb_t1_cajaconcepto BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbT1Cajaconcepto_SELECT", cnx))
                {
                    DataSet ds = new DataSet();
