...
{
    public class tb_pt_categoriaDA
    {
        ...
        public bool Update(string empresaid, tb_pt_categoria BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbPtCategoria_UPDATE", cnx))
                {
        ...
        public DataSet GetAll(string empresaid, tb_pt_categoria BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbPtCategoria_SEARCH", cnx))
                {
        ...
        public DataSet GetOne(string empresaid, string categoriaid)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbPtCategoria_SELECT", cnx))
                {
                    DataSet ds = new DataSet();

                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@categoriaid", SqlDbType.Char, 2).Value = categoriaid;
                    }
