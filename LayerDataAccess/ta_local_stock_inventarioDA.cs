...
{
    public class ta_local_stock_inventarioDA
    {
        ...
        public bool Insert(string empresaid, tb_ta_local_stock_inventario BE)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("gspTbTaLocalStockInventario_INSERT", cnx))
                {
        ...
        public bool Update(string empresaid, tb_ta_local_stock_inventario BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbTaLocalStockInventario_UPDATE", cnx))
                {
        ...
        public bool Delete(string empresaid, tb_ta_local_stock_inventario BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbTaLocalStockInventario_DELETE", cnx))
                {
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@moduloid", SqlDbType.Char, 4).Value = BE.moduloid;
        ...
        public bool GetAll_INICIALIZAR(string empresaid, tb_ta_local_stock_inventario BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbTaLocalStockInventario_INICIALIZAR", cnx))
                {
                    DataSet ds = new DataSet();
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
