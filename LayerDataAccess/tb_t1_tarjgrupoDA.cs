...
{
    public class tb_t1_tarjgrupoDA
    {
        ...
        public bool Insert(string empresaid, tb_t1_tarjgrupo BE)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("gspTbTarjgrupo_INSERT", cnx))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("@tarjgruponame", SqlDbType.VarChar, 30).Value = BE.tarjgruponame;                                                   
                    try
                    {
                        cnx.Open();
        ...
        public bool Update(string empresaid, tb_t1_tarjgrupo BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbTarjgrupo_UPDATE", cnx))
                {
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
        ...
        public DataSet GetAll(string empresaid, tb_t1_tarjgrupo BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbTarjgrupo_SEARCH", cnx))
                {
        ...
        public DataSet GetOne(string empresaid, string ctacte)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbTarjgrupo_SELECT", cnx))
                {
                    DataSet ds = new DataSet();
