...
{
    public class tb_co_tributoafectocomprasDA
    {
        ...
        public bool Insert(string empresaid, tb_co_tributoafectocompras BE)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoTributoafectocompras_INSERT", cnx))
                {                    
        ...
        public bool Update(string empresaid, tb_co_tributoafectocompras BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoTributoafectocompras_UPDATE", cnx))
                {
        ...
        public bool Delete(string empresaid, tb_co_tributoafectocompras BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoTributoafectocompras_DELETE", cnx))
                {
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@destinoid", SqlDbType.Char, 1).Value = BE.destinoid;
        ...
        public DataSet GetAll(string empresaid, tb_co_tributoafectocompras BE)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoTributoafectocompras_SEARCH", cnx))
                {
        ...
        public DataSet GetOne(string empresaid, string destinoid)
        {
            using (SqlConnection cnx = new SqlConnection(conex.empConexion(empresaid)))
            {
                using (SqlCommand cmd = new SqlCommand("gspTbCoTributoafectocompras_SELECT", cnx))
                {
                    DataSet ds = new DataSet();
